...
class TestSubjectConsumerGroupGate(TestCase):
    ...
    async def test_group_gate(self):
        '''
        test access to group gate in a period
        '''
    ...
    @pytest.mark.asyncio
    async def test_group_gate_reset_access(self):
        '''
        test harvest patch
        '''
        ...
        #remove player 1 from group gate 1
        message = {'message_type' : 'group_gate_access_revoke',
                   'message_text' : {"player_id" : p1_id,},
                   'message_target' : 'group', 
                   }
        ...
        group_gate_1 = message_data['group_gates'][str(group_gate_1_id)]
        self.assertNotIn(p1_id, group_gate_1['allowed_players'])
        self.assertEqual(len(group_gate_1['allowed_players']), 0)

        #check that player 2 can access group gate 1
        message = {'message_type' : 'group_gate_access_request',
                   'message_text' : {"player_id" : p2_id,
                                     "group_gate_id" : group_gate_1_id,},
                   'message_target' : 'group', 
        ...
        group_gate_1 = message_data['group_gates'][str(group_gate_1_id)]
        self.assertIn(p2_id, group_gate_1['allowed_players'])
        self.assertEqual(len(group_gate_1['allowed_players']), 1)
