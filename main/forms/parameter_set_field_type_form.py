...
class ParameterSetFieldTypeForm(forms.ModelForm):
  ...
  good_one_ft = forms.ChoiceField(label='Good One',
                              choices=Goods.choices,
                              widget=forms.Select(attrs={"v-model":"current_parameter_set_field_type.good_one_ft",}))

  good_two_ft = forms.ChoiceField(label='Good Two',
                              choices=Goods.choices,
                              widget=forms.Select(attrs={"v-model":"current_parameter_set_field_type.good_two_ft",}))
  ...
  class Meta:
      ...
      fields =['info', 'display_text', 'good_one_ft', 'good_two_ft', 'start_on_period', 'reset_every_n_periods',
                'good_one_alpha', 'good_one_omega', 'good_one_rho', 'good_two_alpha', 'good_two_omega', 'good_two_rho']
  
  def clean_good_one_ft(self):
        
        try:
           good_one_ft = self.data.get('good_one_ft')
           good_two_ft = self.data.get('good_two_ft')

           if good_one_ft == good_two_ft:
               raise forms.ValidationError('Good One and Good Two cannot be the same')
